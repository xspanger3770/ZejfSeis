...
import zejfseis4.exception.ApplicationException;
...
public class EventExplorer extends JFrame {
    ...
    private void buildEventPanel() {
        ...
        btnCalculateMag.addActionListener(e -> {
            String textFieldIntensityText = textFieldIntensity.getText();
            try {
                textFieldMag.setText(f1d.format(ManualEvent.calculateManualMagnitude(event.getDistance(),
                        Double.parseDouble(textFieldIntensityText))));
            } catch (NumberFormatException ex) {
                ZejfSeis4.handleException(new RuntimeApplicationException("Invalid intensity: %s".formatted(textFieldIntensityText)));
            }
        });

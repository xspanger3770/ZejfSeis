...
public interface GeoUtils {
    ...
    public static double probability_v6(double mag, double dist, double depth) {
        double actualDetectionCurve = detectionCurve_v7(dist);
        if (dist > 9000) {
            actualDetectionCurve += 0.3 * Math.cos((dist - (9000 + 14700) / 2.0) / ((14700 - 9000) / Math.PI));
        }
        if (dist > 14700) {
            actualDetectionCurve -= 0.5 * Math.sin((dist - 14700) / (3000.0)) * (1.0 / (1 + 500.0 / depth));
        }

        depth = Math.max(0, depth);

        double depthIncrease = 0.6 / (1 + 500.0 / depth);
        actualDetectionCurve -= depthIncrease;

        double surfaceWavesIncrease = 0.75 / (depth * 0.1 + 1.0);
        surfaceWavesIncrease *= 1.0 / (1.0 + Math.pow(Math.E, -(depth - 4.5) * 3.0));
        actualDetectionCurve -= surfaceWavesIncrease;
    ...
    public static double detectionCurve_v7(double km) {
        double a = 7.86;
        double b = -2.4;
        double c = 845;
        double d = 3.05;
        return (a / (1 + c / ((km + 50.0) * d))) + b;
    }
